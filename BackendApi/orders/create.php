...
error_log("[CREATE_ORDER] --- Request received ---");
...
error_log("[CREATE_ORDER] Inputs: customerID=$customerID, addressID=$addressID, paymentMethod=$paymentMethod, clientTotal=$clientTotal, voucherID=$voucherID, selectedRateID=$selectedRateID, shippingFeeClient=$shippingFeeClient");

// --- Validation ---
if ($customerID <= 0 || $addressID <= 0 || empty($paymentMethod) || $clientTotal <= 0 || empty($items) || !is_array($items) || $selectedRateID <= 0) {
    error_log("[CREATE_ORDER] Validation Failed: Dữ liệu đầu vào không hợp lệ.");
    respond(['isSuccess' => false, 'message' => 'Dữ liệu đầu vào không hợp lệ hoặc thiếu thông tin vận chuyển.'], 400);
}

// --- Bắt đầu Transaction ---
error_log("[CREATE_ORDER] Beginning transaction for customerID: $customerID");
$mysqli->begin_transaction();

// ⭐ SỬA ĐỔI: Thêm catch block cho PriceMismatchException
try {
    // 1. Lấy thông tin khách hàng (Giữ nguyên)
    error_log("[CREATE_ORDER] 1. Fetching customer info...");
    ...
    error_log("[CREATE_ORDER] 1. Customer info found: " . $customer_info['email']);
    
    // 2. Kiểm tra tồn kho (Giữ nguyên)
    error_log("[CREATE_ORDER] 2. Updating stock and calculating subtotal...");
    ...
    foreach ($items as $item) {
        ...
        error_log("[CREATE_ORDER] 2a. Updating stock for variantID: $variantID, quantity: $quantity");
    ...
    error_log("[CREATE_ORDER] 2b. Backend Subtotal calculated: $backendSubtotal");
    
    // --- Bước 3A: XÁC MINH PHÍ VẬN CHUYỂN TRÊN SERVER ---
    error_log("[CREATE_ORDER] 3a. Verifying shipping fee...");
    ...
    error_log("[CREATE_ORDER] 3a. Shipping fee verified: $shippingFee");

    
    // --- Bước 3B: TÍNH FINAL TOTAL (Giữ nguyên) ---
    error_log("[CREATE_ORDER] 3b. Calculating final total...");
    if ($voucherID > 0) {
        ...
        error_log("[CREATE_ORDER] 3b. Voucher discount applied: $voucherDiscount");
    }
    
    $finalTotal = ($backendSubtotal + $shippingFee) - $voucherDiscount;
    $finalTotal = max(0.0, $finalTotal); 
    error_log("[CREATE_ORDER] 3b. Final total calculated (Server): $finalTotal");
    ...
    error_log("[CREATE_ORDER] 3c. Client total vs Server total MATCHED.");

    // --- Bước 4: INSERT ĐƠN HÀNG ---
    error_log("[CREATE_ORDER] 4. Inserting order...");
    $paymentStatus = ('COD' === $paymentMethod) ? 'Unpaid' : 'Pending';
    $initialStatus = ('COD' === $paymentMethod) ? 'Processing' : 'Pending';
    
    // ⭐ SỬA: Xóa khởi tạo $paymentToken và $paymentExpiry ở đây (ĐÃ SỬA TRONG BẢN GỐC CỦA TÔI)
    ...
    // ⭐ SỬA LỖI: Thêm 'i' cho biến $voucherID thứ 2. Chuỗi phải là 'iissdsii' (8 ký tự)
    ...
    error_log("[CREATE_ORDER] 4. Order created with orderID: $orderID. Status: $initialStatus, PaymentStatus: $paymentStatus");
    
    // --- BƯỚC MỚI: INSERT SHIPPING (Giữ nguyên) ---
    error_log("[CREATE_ORDER] 5. Inserting shipping details...");
    ...
    error_log("[CREATE_ORDER] 6. Inserting order details...");
    ...
    error_log("[CREATE_ORDER] 7. Processing voucher usage...");
    if ($voucherID > 0) {
        ...
        if ($voucher_info) {
            ...
            if ($isPrivate) {
                error_log("[CREATE_ORDER] 7a. Updating status for PRIVATE voucherID: $voucherID");
            ...
            error_log("[CREATE_ORDER] 7b. Updating global usedCount for voucherID: $voucherID");
            ...
            if ($stmt_update_global_voucher->affected_rows == 0) {
                 if ($isPrivate) {
                     // Rollback voucher cá nhân nếu hết lượt toàn cục (trường hợp hiếm)
    ...
    error_log("[CREATE_ORDER] 8. Deleting cart items...");
    ...
    // --- BƯỚC 9 (SỬA): XỬ LÝ THANH TOÁN / GỬI EMAIL ---
    error_log("[CREATE_ORDER] 9. Finalizing payment method: $paymentMethod");
    if ($paymentMethod === 'VNPay') {
        
        // ⭐ SỬA: Tạo TxnRef và Expiry SAU KHI có orderID
        $txnRef = "ORDER_" . $orderID . "_" . time(); // Định dạng ĐÚNG: ORDER_ID_TIME
        $paymentExpiry = date('Y-m-d H:i:s', strtotime('+1 hour'));

        error_log("[CREATE_ORDER] 9a. VNPay selected. Generating TxnRef: $txnRef");
        
        // ⭐ SỬA: Cập nhật token và expiry vào đơn hàng
        $stmt_update_token = $mysqli->prepare("UPDATE orders SET paymentToken = ?, paymentExpiry = ? WHERE orderID = ?");
        $stmt_update_token->bind_param("ssi", $txnRef, $paymentExpiry, $orderID);
        $stmt_update_token->execute();
        $stmt_update_token->close();
        
        $mysqli->commit(); 
        error_log("[CREATE_ORDER] 9a. Transaction COMMITTED. Generating VNPay URL...");
        ...
        error_log("[CREATE_ORDER] 9a. VNPay response sent.");

    } else {
        // LOGIC COD (Giữ nguyên)
        error_log("[CREATE_ORDER] 9b. COD selected. Attempting to send email...");
        try {
            $orderDataForEmail = [
                'orderID' => $orderID,
            ...
            error_log("[CREATE_ORDER] 9b. Email send status: " . ($emailSent ? 'Success' : 'Failed'));
        } catch (\Throwable $emailE) {
            error_log("[CREATE_ORDER] 9b. Email Confirmation Failed (Exception): " . $emailE->getMessage());
        }

        $mysqli->commit();
        error_log("[CREATE_ORDER] 9b. Transaction COMMITTED.");
        respond([
            'isSuccess' => true, 
        ...
        error_log("[CREATE_ORDER] 9b. COD response sent.");
    }
    
} catch (PriceMismatchException $pme) {
    // Đây là lỗi 409 (Conflict) do giá hoặc phí ship thay đổi
    $mysqli->rollback();
    error_log("[CREATE_ORDER] Transaction Rollback (409 - PriceMismatchException): " . $pme->getMessage());
    // Gửi về mã lỗi 409
    ...
    $mysqli->rollback();
    error_log("[CREATE_ORDER] Transaction Rollback (500 - Exception): " . $e->getMessage());
